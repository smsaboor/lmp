...
import 'package:lakhimpur_kheri/screens/news/a_news_app/models/news_model.dart';
import 'package:lakhimpur_kheri/screens/news/a_news_app/resources/news_api_provider.dart';
import 'package:lakhimpur_kheri/screens/news/a_news_app/ui/screens/web_view.dart';
...
class ListItemState extends State<ListItemBuild> with TickerProviderStateMixin {
  ...
  _buildItem(context) {
    ...
    image.resolve(ImageConfiguration())
        .addListener(new ImageStreamListener((imageInfo, synchronousCall) {
  ...
  _likeUiLogi() async {
    if (_listLiked.indexOf(widget.model.url) < 0) {
      ...
      await prefs.setStringList("liked", _listLiked);
      await NewsApiProvider().deleteFromFirestore({
        "url": widget.model.url,
        "urlToImage": widget.model.urlToImage,
        "title": widget.model.title,
        "author": widget.model.author,
